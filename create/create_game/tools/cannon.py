...
def cannon_begin_handler(arbiter, space, data):
    if arbiter.shapes[0].collision_type == MOVING_OBJ_COLLISION_TYPE:
        ...
        globals.global_collision_data = {'position': obj.body.position, 'velocity': obj.body.velocity}
        ...
        obj.body.apply_impulse_at_local_point(tuple(launch_dir[:]), (0, 0))
        return False
    return True
...
class Cannon(FixedRect):
    ...
    def add_to_space(self, space):
        ...
        self.shape = cannon
        # Add the body before the shape
        space.add(self.shape.body, cannon)
...
def fan_touching_handler(arbiter, space, data):
    if arbiter.shapes[0].collision_type == MOVING_OBJ_COLLISION_TYPE:
        ...
        obj = arbiter.shapes[0]
        if globals.global_collision_data != None:
            globals.global_collision_data = {'position': obj.body.position, 'velocity': obj.body.velocity}
