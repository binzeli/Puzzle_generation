...
class Bucket(FixedPoly):
    ...
    def add_to_space(self, space):
        ...
        self.shape = bucket
        space.add(self.shape.body, bucket)
    ...
    # # In the Bucket.add_to_space method
    # def add_to_space(self, space):
    #     # Convert numpy.ndarray objects to tuples
    #     vertices = [tuple(self.center_position), tuple(self.v_1), tuple(self.v_2), tuple(self.v_3), tuple(self.v_4)]
    #     bucket = pm.Poly(None, vertices)
    #     bucket.body = self.body  # Ensure the body is set before adding the shape to the space
    #     self.shape = bucket
    #     space.add(self.body, bucket)  # Add both the body and the shape to the space
    #     self.attached_shapes.append(bucket)
