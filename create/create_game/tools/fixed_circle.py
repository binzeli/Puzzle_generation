...
class FixedCircle(FixedObj):
    ...
    # In the FixedCircle.render method
    def render(self, screen, scale=None, anti_alias=False):
        if scale is None:
            scale = 1
        draw_pos = scale * self.flipy(self.pos)
        draw_pos = pymunk.Vec2d(int(draw_pos.x), int(draw_pos.y))  # Create a new Vec2d with integer values

        if anti_alias:
            pg.draw.circle(screen, self.color, draw_pos, self.radius)
        else:
            pg.draw.circle(screen, self.color, draw_pos, self.radius, 0)
