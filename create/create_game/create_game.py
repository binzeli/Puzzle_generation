...
class CreateGame(BaseEnv):
    ...
    def check_collisions(self, tool, all_objs):
        for obj in all_objs:
            if len(tool.shape.shapes_collide(obj.shape).points) > 0:
                return True, (tool, obj)
        return False, ()
    ...
    def motion_stopped(self):
        ...
        vel = np.array([np.sqrt(x.shape.body.velocity.get_length_sqrd()) for x in all_objs])
    ...
    def is_valid_place_tool(self, action, actually_place=False):
        ...
        if self.check_out_of_range(action_pos):
            ...
            collide, obj = self.check_collisions(tool, all_objs)
            if collide:
                placed_obj = False
            else:
                if actually_place:
    ...
    def step(self, action):
        ...
        ################################# add information about collision each step
        collide_objects = []
        
        for obj in self.get_all_objs():
            class_name = obj.__class__.__name__
            if (class_name == 'TargetBall'):
                self.red_ball.append((coords_r2c(obj.shape.body.position.x), coords_r2c(obj.shape.body.position.y)))  
            if (class_name == "MarkerBall"):
                self.blue_ball.append((coords_r2c(obj.shape.body.position.x), coords_r2c(obj.shape.body.position.y)))
            tools = self.get_all_objs()
            tools.remove(obj)
            collide, objects = self.check_collisions(obj,  tools)
            if collide:
                collide_objects.append(objects)
        # Add all possible log data to the info array
        info['frames'] = obs
        info['cur_goal_hit'] = self.goal_hit
        info['collide_objects'] = collide_objects
        info['overlap'] = overlap
        info['ep_target_hit'] = self.target_hit
        info['goal'] = (coords_r2c(self.goal_obj.body.position.x), coords_r2c(self.goal_obj.body.position.y))
        ...
        if done:
            ...
            info['red_ball'] = self.red_ball
            info['blue_ball'] = self.blue_ball
